...
export type RelationshipHitTestFn = (
  node: VisualRelationship,
  distance: number
) => boolean;
...
export class VisualGraph {
  constructor(
    ...
    readonly nodes: Record<string, VisualNode>,
    readonly relationshipBundles: RoutedRelationshipBundle[]
  ) {}
  ...
  nodeAtPoint(point: Point) {
    return this.closestNode(
      point,
      (visualNode: VisualNode, distance: number) => {
