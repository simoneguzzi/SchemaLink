...
const intersectVertical = (
  ...
  return {
    ...
    intersection: new Point(
      vertical.center.x,
      Math.tan(other.angle) * (vertical.center.x - other.center.x) +
        other.center.y
    ),
  };
};
...
const intersectVerticalLineAndCircle = (
  line: LineGuide,
  circle: CircleGuide,
  ...
  if (dx > circle.radius) {
    ...
    const y =
      circle.center.y < naturalPosition.y
        ? circle.center.y + dy
        : circle.center.y - dy;
...
export const intersectLineAndCircle = (
  line: LineGuide,
  circle: CircleGuide,
  naturalPosition: Point
): Intersection => {
  ...
  if (d >= 0) {
    ...
    const errors = intersections.map((point) =>
      point.vectorFrom(naturalPosition).distance()
    );
    const intersection =
      errors[0] < errors[1] ? intersections[0] : intersections[1];
    return {
      possible: true,
      intersection,
    };
...
export const intersectCircleAndCircle = (
  circleA: CircleGuide,
  circleB: CircleGuide,
  naturalPosition: Point
): Intersection => {
  ...
  if (
    d > Math.abs(circleA.radius - circleB.radius) &&
    d < circleA.radius + circleB.radius
  ) {
    const a =
    ...
    const intersections = [
      midPoint.translate(bisector),
      midPoint.translate(bisector.invert()),
    ];
    const errors = intersections.map((point) =>
      point.vectorFrom(naturalPosition).distance()
    );
