...
export class LineGuide {
  ...
  calculateError(naturalPosition: Point) {
    const yAxisPoint = naturalPosition
      .translate(this.center.vectorFromOrigin().invert())
      .rotate(-this.angle);
  ...
  point(scalar: number) {
    return new Point(scalar, 0)
      .rotate(this.angle)
      .translate(this.center.vectorFromOrigin());
  }
  ...
  intervalGuide(nodes: { position: Point }[], naturalPosition: Point) {
    ...
    const intervals = coLinearIntervals(
    ...
    if (intervals.length > 0) {
      ...
      return new LineGuide(
        this.point(interval.candidate),
